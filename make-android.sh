...
adb push ./ /data/local/tmp/cl/

adb shell "cd /data/local/tmp/cl ; LD_LIBRARY_PATH=/data/local/tmp/cl/android-libs sh make-target-2.sh"

# Hack needed to replace SB-GROVEL:RUN-C-COMPILER
compile_one() {
    bin=temp-compile-from-android
    adb pull /data/local/tmp/cl/contrib/asdf/$2 $bin.c
    $CC $bin.c -o $bin
    dest=$(echo $3 | sed 's/\r//g')  # On older android line terminates with \r
    adb push $bin /data/local/tmp/cl/contrib/asdf/$dest
    echo "done"
    rm $bin
    rm $bin.c
}

adb shell "cd /data/local/tmp/cl ; LD_LIBRARY_PATH=/data/local/tmp/cl/android-libs sh make-target-contrib-android.sh" | \
    while read line ;
      do echo "$line" ;
      echo $line | grep "RUN-C-COMPILER" | while read line ; do compile_one $line ; done ;
    done

adb pull /data/local/tmp/cl/obj
adb pull /data/local/tmp/cl/output
