...
run_cl --script $tmpscript 1> $tmpout 2> $tmperr
...
EOF
echo ONE | run_cl --script $tmpscript 1> $tmpout 2> $tmperr
...
(loop (write-line "foo"))
EOF
run_cl --script $tmpscript 2> $tmperr | head -n1 > $tmpout
...
echo '(write-line "Ok!")' | run_cl --script 1>$tmpout 2>$tmperr
...
(print :script-ok)
EOF
run_cl --script $tmpscript --eval foo \
  < /dev/null > $tmpout
...
run_cl --eval "(compile-file \"$tmpscript\" :output-file \"$tmpfasl\")"  </dev/null >/dev/null
chmod +x $tmpfasl
